import { b as createAstro, c as createComponent, r as renderComponent, a as renderTemplate, m as maybeRenderHead, d as addAttribute } from '../../chunks/astro/server_0kCClvUH.mjs';
...
</p> </div> </div> <!-- boxer container (luchador) --> <div class="relative w-[40%] min-w-[250px] h-[450px] flex flex-row justify-center items-center"> <!-- Luchador --> <img${addAttribute(`Imagen de ${name}`, "aria-label")}${addAttribute(`hero-image-${id}`, "data-id")}${addAttribute(`/images/fighters/big/${id}.webp`, "src")}${addAttribute(fighter?.name, "alt")} class="absolute w-auto h-[550px] mb-24 z-0 hover:scale-125 transition-[.6s]"> <!-- Nombre --> <img${addAttribute(`Nombre de ${name}`, "aria-label")}${addAttribute(`hero-text-${id}`, "data-id")}${addAttribute(`/images/fighters/text/${id}.webp`, "src")}${addAttribute("Nombre de " + name, "alt")} class="relative h-auto w-full max-w-[400px] z-10"> </div> </div> <!-- contenedor de la descripción --> <div class="w-[45%]"> <div class="relative w-[65%] h-[270px] mx-auto mt-2! border-2 rounded-2xl overflow-hidden flex justify-end"> <img class="absolute w-32 top-0 h-auto object-cover opacity-100 z-50 bg-gray-500"${addAttribute(`/images/fighters/cards/${id}.webp`, "src")}${addAttribute(`Tarjeta del boxeador ${name}`, "alt")}> </div> </div> </div> </section> ` })}`;
}, "C:/Users/axl_7/Escritorio/developer/la-desvelada-vercel/src/pages/luchador/[id].astro", void 0);
